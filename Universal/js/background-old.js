...
browser.storage.local.get('first', (res) => {
...
function init() {
    ...
    console.log('初始化')
    // 写入初始信息
    browser.storage.local.set({ first: 'first' });

    //默认搜索引擎
    browser.storage.local.set({ engine: 0 });
    browser.storage.local.set({ default_engine_url: "http://www.baidu.com/s?wd=%s" });
...
browser.menus.onClicked.addListener(function (params, tab) {
    if (params.menuItemId === "menu-item-id") {
        // 输出
